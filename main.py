...
def astar(heuristic):
	...
	while not opqueue.empty():
		...
		for n in current.neighbors():
			...
			if n.type == "leap" or n.type == "fw":
				new_cost = cost_so_far[current] + current.cost(n) + heuristic(new_node)
			
			else:
				new_cost = cost_so_far[current] + current.cost(n)
...
#heuristic1 = 0
if args.method == 1:
	cameFrom, cost_so_far, real_cost = astar(lambda x: 0)
	
#heuristic2 = minimum (vertical or horizontal distance from the node to goal)
if args.method == 2:
	cameFrom, cost_so_far, real_cost = astar(lambda x: min(abs(g[0]-x.pos_r),abs(g[1]-x.pos_c)))
	
#heuristic3 = maximum (vertical or horizontal distance from the node to goal)
if args.method == 3:
	cameFrom, cost_so_far, real_cost = astar(lambda x: max(abs(g[0]-x.pos_r),abs(g[1]-x.pos_c)))

#heuristic4 = Manhattan distance
if args.method == 4:
	cameFrom, cost_so_far, real_cost = astar(lambda x: abs(g[0]-x.pos_r) + abs(g[1]-x.pos_c))

#heuristic5 = Manhattan distance if previous step is fw, else if (turn then added the cost)
if args.method == 5:
	cameFrom, cost_so_far, real_cost = astar(lambda x: (x.pos_c + x.pos_r) if (x.prevstep == 'Forward') else ((x.pos_c + x.pos_r + x.getcost(3)) if (x.prevstep =='leap') else(x.pos_c + x.pos_r+x.getcost(0)/3))) 

#heuristic6 = heuristic5*3
if args.method == 6:
	cameFrom, cost_so_far, real_cost = astar(lambda x: (3*(x.pos_c + x.pos_r)) if (x.prevstep == 'Forward') else ((3*(x.pos_c + x.pos_r + x.getcost(3))) if (x.prevstep =='leap') else(3*(x.pos_c + x.pos_r+x.getcost(0)/3)))) 


goal = next(filter(lambda x: x.isGoal(), cameFrom.keys()))
path = construct_path(cameFrom, Robot(), goal)
print("Score: " + str(score - real_cost[goal]))
